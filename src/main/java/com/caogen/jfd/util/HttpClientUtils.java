...
public class HttpClientUtils {
	...
	private static String getGetUrlFromParams(String uri, Map<String, String> params) {
		...
		List<BasicNameValuePair> resultList = FluentIterable.from(params.entrySet())
				.transform(new Function<Map.Entry<String, String>, BasicNameValuePair>() {
					@Override
	...
	private static PoolingHttpClientConnectionManager initConfig() {
		...
		/**
		 * 以下参数设置含义分别为: 1 是否立即发送数据，设置为true会关闭Socket缓冲，默认为false 2
		 * 是否可以在一个进程关闭Socket后，即使它还没有释放端口，其它进程还可以立即重用端口 3 接收数据的等待超时时间，单位ms 4
		 * 关闭Socket时，要么发送完所有数据，要么等待多少秒后，就关闭连接，此时socket.close()是阻塞的 5 开启监视TCP连接是否有效
		 * 其中setTcpNoDelay(true)设置是否启用Nagle算法，设置true后禁用Nagle算法，默认为false（即默认启用Nagle算法）。
		 * Nagle算法试图通过减少分片的数量来节省带宽。当应用程序希望降低网络延迟并提高性能时， 它们可以关闭Nagle算法，这样数据将会更早地发
		 * 送，但是增加了网络消耗。 单位为：毫秒
		 */
